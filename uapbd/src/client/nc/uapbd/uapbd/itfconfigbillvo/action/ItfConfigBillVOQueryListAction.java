...
import nc.uapbd.uapbd.itfconfigbillvo.uapbd.bean.PageQueryVO;
 ...
 */
public class ItfConfigBillVOQueryListAction implements ICommonAction {
  ...
  private Grid covert(PageQueryVO pagevo, int pageSize, String pagecode) {
    AggItfConfigBillVO[] bills =
        (AggItfConfigBillVO[]) pagevo.getCurrentPageBills();
  ...
  private PageQueryVO createNullPage() {
    ...
    AggItfConfigBillVO[] bills =
        Constructor.construct(AggItfConfigBillVO.class, 0);
   ...
   * @return
   */
  private AggItfConfigBillVO[] getDefaulePageBill(QueryTreeFormatVO info,
      AggItfConfigBillVO... aggvos) {
    ...
    List<AggItfConfigBillVO> billLst = new ArrayList<AggItfConfigBillVO>();
    for (int i = 0; (i < aggvos.length) && (i < pageSize); i++) {
      AggItfConfigBillVO bill = new AggItfConfigBillVO();

      bill.setParent(aggvos[i].getParent());
      billLst.add(bill);
    }
    return billLst.toArray(new AggItfConfigBillVO[0]);
  }
   ...
   * @return
   */
  private String[] getPks(AggItfConfigBillVO... aggvos) {
    List<String> pks = new ArrayList<String>();
    for (AggItfConfigBillVO bill : aggvos) {
