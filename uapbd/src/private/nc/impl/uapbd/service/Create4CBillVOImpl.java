 ...
 * 库存销售出库单
 */
public class Create4CBillVOImpl implements ICreate4CBillVO {
	...
	public AggregatedValueObject createBillVO(HashMap<String, Object> nccBillMap, String pk_group) throws BusinessException {
		...
		//select * from md_class where defaulttablename='ic_saleout_h' or  defaulttablename='ic_saleout_b'
		String headclassid = "38a23a56-0686-4959-b094-16519b5bcb13";//销售出库单表头元数据ID，需要通过语句查找最新版本元数据
		String bodyclassid = "9b54baf6-265f-48d3-8d5f-b25c5769b945";//销售出库单表体元数据ID，需要通过语句查找最新版本元数据
		//组装单据VO
		SaleOutVO billvo = new SaleOutVO();
		try {
			...
			AssertUtils.listIsNull(bodyList, "没有获取到表体信息！");
				...
				SaleOutHeadVO headvo = new SaleOutHeadVO();
				...
				for (String key:headMap.keySet()) {
					...
					if (existvalue == null || "".equals(existvalue))
						continue;
					// 转换字段值
					Object value = TranslateUtils.getTranslateValue(headclassid, pk_group, pk_org, key, existvalue, "4C_H",
							itftype);
				...
				headvo.setCreceivfinorgoid(pk_financeorg);//应收财务组织最新版本  
				headvo.setCreceivfinorgvid(pk_financeorg_v);
				headvo.setCsaleorgoid(pk_org);//销售组织最新版本  
				headvo.setCsaleorgvid(pk_org_v);
				
				headvo.setCsendcountryid(pk_country);//发货国 
				headvo.setCtaxcountryid(pk_country);//报税国  
				headvo.setFbuysellflag(1);//购销类型  1=国内销售，2=国内采购，3=出口，4=进口，5=不区分，   
				...
				ArrayList<SaleOutBodyVO> bodyvolist = new ArrayList<SaleOutBodyVO>();
				
				for (int j = 0; j < bodyList.size(); j++) {
					
					HashMap<String,String> bodymap = (HashMap<String,String>)bodyList.get(j);
					SaleOutBodyVO bodyvo = new SaleOutBodyVO();
...
//					bodyvo.setVqtunitrate(bodyvo.getVchangerate());//报价换算率
					...
					//casscustid  客户  
					bodyvo.setCasscustid(headvo.getCcustomerid());//客户  
...
//					bodyvo.setNnum(new UFDouble(1));//实收主数量
					//应发主数量项目主键	nshouldnum
					bodyvo.setNshouldnum(bodyvo.getNshouldassistnum());
					//实发主数量项目主键	nnum
					bodyvo.setNnum(bodyvo.getNassistnum());
					//2020年2月2日12:02:12注释
					//  norigtaxmny  价税合计  
//					bodyvo.setNorigtaxmny((bodyvo.getNtax()==null?UFDouble.ZERO_DBL:bodyvo.getNtax()).add(bodyvo.getNorigmny()));
					//ntaxmny  本币价税合计  
//					bodyvo.setNtaxmny((bodyvo.getNtax()==null?UFDouble.ZERO_DBL:bodyvo.getNtax()).add(bodyvo.getNmny()));
				...
				billvo.setChildrenVO(bodyvolist.toArray(new SaleOutBodyVO[0]) );
				
				
			
		} catch (Exception e) {
			e.printStackTrace();
			throw new BusinessException("转换库存销售出库数据VO出错！"+e.getMessage());
		}
		//查看外部交换平台代码
		//检查是否允许保存
