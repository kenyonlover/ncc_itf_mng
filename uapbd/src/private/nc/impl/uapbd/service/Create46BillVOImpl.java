 ...
 * 库存采购入库单
 */
public class Create46BillVOImpl implements ICreate46BillVO {
	...
	public AggregatedValueObject createBillVO(HashMap<String, Object> nccBillMap, String pk_group)
		...
		// select * from md_class where defaulttablename='ic_finprodin_h' or
		// defaulttablename='ic_finprodin_b'
		String headclassid = "790b7fb2-be31-49b3-9e0f-070d641dca88";// 库存产成品入库单表头元数据ID，需要通过语句查找最新版本元数据
		String bodyclassid = "7c3aa22d-59c3-4df3-89f7-3908d9809290";// 库存产成品入库单表体元数据ID，需要通过语句查找最新版本元数据
		// 组装单据VO
		FinProdInVO billvo = new FinProdInVO();
		try {

			String pk_ountry = CreateBillUtils.getPk_Countryzone("CN");// 获取国家（中国）
			String pk_currtype = CreateBillUtils.getPk_Currtype("CNY");// 币种

			String itftype = (String) nccBillMap.get("itftype");
			...
			FinProdInHeadVO headvo = new FinProdInHeadVO();
			...
			for (String key : headMap.keySet()) {
				...
				if (existvalue == null || "".equals(existvalue))
					continue;
				// 转换字段值
				Object value = TranslateUtils.getTranslateValue(headclassid, pk_group, pk_org, key, existvalue, "46_H",
						itftype);
			...
			headvo.setCprocalbodyoid(pk_org);// 生产库存组织最新版本
			headvo.setCprocalbodyvid(pk_org_v);

			headvo.setCprowarehouseid(headvo.getCwarehouseid());// 生产仓库
...
//				headvo.setCtrantypeid(trantypeid);//出入库类型
			...
			ArrayList<FinProdInBodyVO> bodyvolist = new ArrayList<FinProdInBodyVO>();

			for (int j = 0; j < bodyList.size(); j++) {

				HashMap<String, String> bodymap = (HashMap<String, String>) bodyList.get(j);
				FinProdInBodyVO bodyvo = new FinProdInBodyVO();

				for (String key : bodymap.keySet()) {
					String exsvalue = bodymap.get(key);
					if (exsvalue == null || "".equals(exsvalue))
						continue;
					// 转换字段值
					Object value = TranslateUtils.getTranslateValue(bodyclassid, pk_group, pk_org, key, exsvalue,
							"46_B", itftype);
...
//					bodyvo.setCfanaceorgoid(headvo.getCfanaceorgoid());//结算财务组织
				...
				// cproductid 主产品
				bodyvo.setCproductid(bodyvo.getCmaterialoid());
...
//					bodyvo.setNnum(new UFDouble(1));//实收主数量
			...
			billvo.setChildrenVO(bodyvolist.toArray(new FinProdInBodyVO[0]));

		} catch (Exception e) {
			e.printStackTrace();
			throw new BusinessException("转换库存产成品入库数据VO出错！" + e.getMessage());
		}
		// 查看外部交换平台代码
		// 检查是否允许保存
