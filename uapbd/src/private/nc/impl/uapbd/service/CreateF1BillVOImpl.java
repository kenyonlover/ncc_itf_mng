...
public class CreateF1BillVOImpl implements ICreateF1BillVO {
	...
	public AggregatedValueObject createBillVO(HashMap<String, Object> nccBillMap, String pk_group)
		...
		// select * from md_class where defaulttablename='ap_payablebill' or
		// defaulttablename='ap_payableitem'
		String headclassid = "6c8584f4-21d4-4ec9-9eac-96ad472acf2f";// 应付单表头元数据ID，需要通过语句查找最新版本元数据
		String bodyclassid = "091a5f27-e8b7-4538-912d-221730d0ba1c";// 应付单表体元数据ID，需要通过语句查找最新版本元数据

		// 组装单据VO
		AggPayableBillVO billvo = new AggPayableBillVO();
		try {
			...
			PayableBillVO headvo = new PayableBillVO();
			...
			ArrayList<PayableBillItemVO> bodyvolist = new ArrayList<PayableBillItemVO>();
			...
			for (int j = 0; j < bodyList.size(); j++) {
				...
				bodyvo.setObjtype(1);// 往来对象 3=业务员，2=部门，1=供应商，
...
//				headvo.setNtotalnum(totalnum);//总数量
			...
			setBodyDefault(headvo, bodyvolist.toArray(new PayableBillItemVO[0]));
			billvo.setParentVO(headvo);
			billvo.setChildrenVO(bodyvolist.toArray(new PayableBillItemVO[0]));

		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw new BusinessException("转换应付单数据VO出错！" + e.getMessage());
		}
	 ...
	 * @throws BusinessException
	 */
	private PayableBillVO setHeaderDefault(PayableBillVO header) throws BusinessException {
...
//		header.setSrc_syscode(BillEnumCollection.FromSystem.WBJHPT.VALUE);		
	...
	private void setBodyDefault(PayableBillVO head, PayableBillItemVO[] items) throws BusinessException {
	...
	private void setBodyDefault(PayableBillVO head, PayableBillItemVO item) throws BusinessException {
		/* 复制单位 */
