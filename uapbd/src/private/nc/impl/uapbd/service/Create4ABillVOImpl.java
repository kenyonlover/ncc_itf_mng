 ...
 * 库存其他入库单
 */
public class Create4ABillVOImpl implements ICreate4ABillVO {
	...
	public AggregatedValueObject createBillVO(HashMap<String, Object> nccBillMap, String pk_group)
		...
		// select * from md_class where defaulttablename='ic_generalin_h' or
		// defaulttablename='ic_generalin_b'
		String headclassid = "966630e3-3cf1-4275-a98e-51bb3d2172c4";// 库存其他入库单表头元数据ID，需要通过语句查找最新版本元数据
		String bodyclassid = "40883166-41f3-4243-a0f1-b649da84977b";// 库存其他入库单表体元数据ID，需要通过语句查找最新版本元数据
		GeneralInVO billvo = new GeneralInVO();
		try {
			...
			for (String key : headMap.keySet()) {
				...
				if (existvalue == null || "".equals(existvalue))
					continue;
				// 转换字段值
				Object value = TranslateUtils.getTranslateValue(headclassid, pk_group, pk_org, key, existvalue, "4A_H",
						itftype);
			...
			ArrayList<GeneralInBodyVO> bodyvolist = new ArrayList<GeneralInBodyVO>();
			...
			billvo.setChildrenVO(bodyvolist.toArray(new GeneralInBodyVO[0]));

		} catch (Exception e) {
			e.printStackTrace();
			throw new BusinessException("转换库存其他入库单数据VO出错！" + e.getMessage());
		}
		// 查看外部交换平台代码
		// 检查是否允许保存
